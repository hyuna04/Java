...
public class _04_pipeLineEx {
	...
	public static void main(String[] args) {
		...
		cardList.add(
				new CreditCard("현대", "더블랙", 100000)
		);
		cardList.add(
				new CreditCard("아멕스", "블랙", 300000)
		);
		//방법 1
		...
		//방법 2
		cardList.stream()//creditCard 객체 여러개 저장된 스트림
				//List<CreditCard> -> Stream<CreditCard>
				.filter(card ->
						card.getLimitMoney() >= 5000)
				//filter<CreditCard> = [{삼성, id, 5000},
				//						{현대, 더블랙, 10000},
				//						{아멕스, 블랙, 30000}]
				.forEach(c ->
						System.out.println(c.getLimitMoney()
								+ " : " + c.getCardName()));
